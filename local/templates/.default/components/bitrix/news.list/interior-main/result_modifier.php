...
if(sizeof($arIds) > 0)
{
    ...
    while($arItem = $rsFile->GetNext())
    {
        ...
        $urlPreviewPicture = \itc\Resizer::get($arItem['ID'], 'interiorPreview');
        $urlDetailPicture  = \itc\Resizer::get($arItem['ID'], 'interiorDetail');
    ...
    foreach($arResult['ITEMS'] as &$arItem)
    {
        ...
        {
            $arItem['PREVIEW_PICTURE']['SRC'] = \itc\Resizer::get($environment->get('interiorPlugId'), 'interiorPreview');
            $arItem['DETAIL_PICTURE']['SRC'] = \itc\Resizer::get($environment->get('interiorPlugId'), 'interiorDetail');
        }
    }
    unset($arItem);
}
