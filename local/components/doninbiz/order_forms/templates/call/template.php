...
$sAction = SITE_DIR . 'order/call.php';
...
<?if(empty($iHideModal)): ?>
<div class="modal fade modal-order-form" id="orderCallModal" tabindex="-1" role="dialog" aria-labelledby="orderCallFormLabel" aria-hidden="true" data-uri="<?=$sAction?>">
                ...
                <div class="m-outer-form" id="orderCall">
                    ...
                    <div class="m-header">
                        <?if($arParams['FORM_NAME']):?>
                            <h3 id="orderCallFormLabel"><?=$arParams['FORM_NAME']?></h3>
                        <?endif;?>
                    ...
                    <div class="m-content">
                        ...
                        <?else:?>
                            ...
                            <form method="post" action="<?=$sAction?>" id="orderCallForm"<?=$sSubmitGoal?>>
                                <?=bitrix_sessid_post()?>
                                ...
                                <div class="form-group<?=(!empty($arResult['ERROR']['NAME']) && !empty($_POST) ? ' has-error' : '');?>">
                                    <label for="form-CALL-NAME"><?=GetMessage('FORM_CALL_FIELD_NAME')?>: <span class="require">*</span></label>
                                    <input type="text" class="form-control" id="form-CALL-NAME" name="NAME" value="<?=$arResult['DATA']['NAME'];?>" />
                                ...
                                <div class="form-group<?=(!empty($arResult['ERROR']['MOBILE']) && !empty($_POST) ? ' has-error' : '');?>">
                                    <label for="form-CALL-MOBILE"><?=GetMessage('FORM_CALL_FIELD_MOBILE')?>: <span class="require">*</span></label>
                                    <span class="phone pull-right"></span>
                                    <input type="text" class="form-control" id="form-CALL-MOBILE" name="MOBILE" value="<?=$arResult['DATA']['MOBILE'];?>" />
                                ...
                                <?if($arResult['USE_CAPTCHA']):?>
                                    <?
                                    $dynamicArea = new \Bitrix\Main\Page\FrameStatic("modal_captcha_call");
                                    $dynamicArea->startDynamicArea();
                                    ?>
                                    <div class="form-group<?=(!empty($arResult['ERROR']['CAPTCHA']) && !empty($_POST) ? ' has-error' : '');?>">
                                        <label for="form-CALL-CAPTCHA"><?=GetMessage('FORM_CALL_FIELD_CAPTCHA')?>: <span class="require">*</span></label>
                                        <div class="captcha-img">
                                            <input type="hidden" name="captcha_sid" value="<?=$arResult["capCode"]?>">
                                            <img src="/bitrix/tools/captcha.php?captcha_sid=<?=$arResult["capCode"]?>" width="180" height="40" alt="CAPTCHA">
                                            <a href="#" class="update-captcha-img"><i class="fa fa-refresh"></i> <?=GetMessage('FORM_CALL_CAPTCHA_CHANGE_IMAGE')?></a>
                                        </div>
                                ...
                                <div class="m-footer">
                                    <div class="left">
                                        <div class="required">
                                            <span>*</span> <?=GetMessage('FORM_CALL_REQUIRED_FIELDS')?>
                                        </div>
                                    </div>
                                    <div class="right">
                                        <button type="submit" class="btn btn-primary btn-lg btn-submit" data-loading-text="<?=GetMessage('FORM_CALL_SEND')?>"<?=$sClickGoal?>>
